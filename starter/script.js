...
const startLogOutTimer = function () {
  const tick = function () {
    ...
    // In each call, print the emaining time to UI
    labelTimer.textContent = `${min}:${sec}`;

    if (time === 0) {
      clearInterval(timer);
      labelWelcome.textContent = 'Log in to get started';
      // this is going to disapear the screen, so the user seems to be loged out
    ...
    // When timer gets to 0 second ,stop the timer and log out the user
  };
...
//   minute: 'numeric',
//   day: 'numeric',
//   // month: 'long' -> This will show the full Months' name
//   month: 'short',
//   year: 'numeric',
//   weekday: 'long',
// };
// const locale = navigator.language;
// console.log(`LANGUAGE WAS SET on My COMPUTER: ${locale}`);

// labelDate.textContent = new Intl.DateTimeFormat('en-GB').format(now);
// result: As of 01/01/2022

//This will use the date for Iran
// labelDate.textContent = new Intl.DateTimeFormat('fa-IR', options).format(now);

// labelDate.textContent = new Intl.DateTimeFormat(locale, options).format(now);

// ISO labguage code table
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === +inputLoginPin.value) {
    ...
    // If there is a timer for any user that logged in, it will clear that
    // This will help when we jump from one account to another, therefore the timer will work properly
